...
module.exports = function(args) {
    ...
    spawn('git', ['ls-remote', '--tags'], {
        ...
        return fs.exists(deployDir).then(function(exist) {
            spinner.text = '🚀 ' + chalk.black.bgCyan(' Start! ') + chalk.cyan(' Git ' + dirName + ' Tag deployment...');
            if (exist) return;
        ...
        .then(function() {
            ...
            return util.git('remote', 'add', 'origin', repo).then(() => {
                return util.git('checkout', '-b', dirName);
            }).then(() => {
                return util.git('add', '-A');
            }).then(() => {
                return util.git('commit', '-m', message || 'git push ' + dirName + ' tag: ' + tag);
            }).then(() => {
                util.git('fetch');
                return util.git('push', '-u', 'origin', dirName, '-f');
            }).then(() => {
